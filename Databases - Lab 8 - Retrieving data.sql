...
-- Question 1: List each club name and the number of players currently under contract. Show only clubs that have more than 10 players,
-- and sort the results from the highest to the lowest number of players.
SELECT c.club_name,
COUNT(pc.player_id) AS num_players
FROM clubs c 
...
-- Question 2: Display the player name, club name, and number of years remaining on their contract.
-- Only include players whose contracts have not yet expired. Be ready to explain the results when you demonstrate your lab.
select p.player_name, c.club_name, pc.contract_valid_until, pc.contract_valid_until - extract(year from current_date) as years_remaining
from players p join player_contracts pc using (player_id)
join clubs c using (club_id)
where pc.contract_valid_until >= extract(year from current_date);

-- Question 3: Find the average wage for each nationality.
-- Show only the nationalities whose average wage is higher than the overall average wage across all players.
SELECT p.nationality, AVG(pc.wage) AS avg_wage
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
GROUP BY p.nationality
HAVING AVG(pc.wage) > (SELECT AVG(wage) FROM player_contracts)
ORDER BY avg_wage DESC;

-- Question 4: List the player names and club names for players who play in a club that
-- has at least one player earning €50,000 or more per week.
SELECT DISTINCT p.player_name, c.club_name
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
...
-- Question 5: Return the names of clubs that currently have no players under contract.
SELECT c.club_name
FROM clubs c
LEFT JOIN player_contracts pc ON c.club_id = pc.club_id
WHERE pc.player_id IS NULL;

-- Question 6: Write a query to achieve the following output:
-- - The first column should display each player’s name
-- - The second column should show their preferred foot
-- - The third column should show values “Left-footed” if the player’s preferred foot is Left, “Right-footed” if
-- the preferred foot is Right, and “Unknown” if the preferred foot is NULL.
SELECT player_name, preferred_foot,
CASE 
WHEN preferred_foot = 'Left' THEN 'Left-footed'
WHEN preferred_foot = 'Right' THEN 'Right-footed'
ELSE 'Unknown'
END AS foot_description
FROM players;