...
-- a. Write a query to list all players from Germany. Show player_name, age and nationality. Check how long this query takes to run.
EXPLAIN ANALYZE
SELECT player_name, age, nationality
FROM players
WHERE nationality = 'Germany';

-- b. Now, create an index on players(nationality) and re-run the query you wrote for part a.
...
-- c. Write a comment on your SQL script explaining if there were any improvements after creating the index.
-- After creating the index, the query’s execution time decreased significantly since the database used an index scan instead of a sequential one, improving speed for large datasets


-- Question 2: The finance team frequently wants a report with the top-paid players across all clubs.
-- a. Write a query that returns player_name, club_name, and wage. Use EXPLAIN ANALYZE to check how long that query takes to run.
EXPLAIN ANALYZE
SELECT p.player_name, c.club_name, pc.wage
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
JOIN clubs c ON pc.club_id = c.club_id
ORDER BY pc.wage DESC;

-- b. Create an index on the wage column and re-run the query you wrote for part a.
CREATE INDEX idx_playercontracts_wage ON player_contracts(wage);
EXPLAIN ANALYZE
SELECT p.player_name, c.club_name, pc.wage
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
...
-- c. Write a comment on your SQL script explaining if there were any improvements after creating the index.
-- After creating the index, the query execution time reduced, since the database used an index scan for sorting wages, improving performance for large tables.


-- Question 3: Your manager wants details on expensive players, with market_value above 5,000,000.
-- a. Write a query to list all players whose market)value > 5,000,000, displaying the player_name, club_name, value and wage. Use EXPLAIN ANALYZE to check how long that query takes to run.
EXPLAIN ANALYZE
SELECT p.player_name, c.club_name, pc.value, pc.wage
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
JOIN clubs c ON pc.club_id = c.club_id
WHERE pc.value > 5000000
ORDER BY pc.value DESC;

-- b. Create a partial index that only stores rows with market_value above 5,000,000. Re-run the query you wrote for part a.
CREATE INDEX idx_playercontracts_value_expensive
ON player_contracts(value)
WHERE value > 5000000;
EXPLAIN ANALYZE
SELECT p.player_name, c.club_name, pc.value, pc.wage
FROM players p
...
-- c. Write a comment on your SQL script explaining if there were any improvements after creating the index.
-- After creating the index, it improved query performance and reduced space usage, since the query now uses an index scan for expensive players, making it faster compared to the sequential scan before the index.
...
'idx_playercontracts_wage',
'idx_playercontracts_value_expensive'
);