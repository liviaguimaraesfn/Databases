...
-- 1. Using SQL SET operators, how would you show thich player_id values appear in player_contracts but not in players?
-- using EXCEPT operator that returns rows in the first query that are not present in the second query
SELECT player_id FROM player_contracts
except
SELECT player_id FROM players;

-- 2. Using SQL SET operators, how would you show which clubs are used in player contracts but do not exist in the table clubs?
-- using UNION operator that combines results and removes duplicates
SELECT club_id FROM player_contracts
UNION
SELECT club_id FROM clubs;

-- 3. Using a SET operator, list the IDs of players under 20 years old who already have a contract.
-- using INTERSECT operator that returns rows that have common values to both queries
SELECT player_id FROM players
WHERE age < 20
INTERSECT
SELECT player_id FROM player_contracts;

-- 4. Using a SET operator, identify players who have an overall rating equal to or greater than 85 but still do not have a contract with any club.
-- using EXCEPT operator that returns rows in the first query that are not present in the second query
...
-- 5. Using a SET operator, identify player IDs present in the tables players, player_contracts, and player_attributes. (Hint: you might need to repeat the same set operator)
-- using INTERSECT operator that returns rows that have common values to both queries
SELECT player_id FROM players
INTERSECT
SELECT player_id FROM player_contracts
