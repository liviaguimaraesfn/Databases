...
-- Request 1) List the names of all players and of the clubs they are currently contracted to.
...
-- Request 2) Find all players who have the attributes overall and potential recorded but no contract with any club.
...
WHERE pc.club_id IS NULL
  AND pa.overall IS NOT NULL
  AND pa.potential IS NOT NULL;

-- Request 3) List the top 10 highest-paid players, showing their name, wage, and the club they play for
-- (Hint: if you add LIMIT 10 to the end of your code, only the first 10 rows will be shown).
SELECT p.player_name, pc.wage, c.club_name
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
JOIN clubs c ON pc.club_id = c.club_id
...
-- Request 4) Show the player name, club name, and contract_valid_until for players whose contracts expire in 2025.
SELECT p.player_name, c.club_name, pc.contract_valid_until
FROM players p
JOIN player_contracts pc ON p.player_id = pc.player_id
JOIN clubs c ON pc.club_id = c.club_id
WHERE pc.contract_valid_until = 2025;
